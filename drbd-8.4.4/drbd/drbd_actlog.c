...
bool drbd_al_begin_io_prepare(struct drbd_device *device, struct drbd_interval *i)
{
	...
	for (enr = first; enr <= last; enr++) {
		struct lc_element *al_ext;
		atomic_set(&device->debug_noblock,8);
		wait_event(device->al_wait,
				(al_ext = _al_get(device, enr, false)) != NULL);
		if (al_ext->lc_number != enr)
			need_transaction = true;
	}
	atomic_set(&device->debug_noblock,9);
...
void drbd_al_begin_io_commit(struct drbd_device *device, bool delegate)
{
	 ...
	 */
	atomic_set(&device->debug_noblock,1);
	wait_event(device->al_wait,
			device->act_log->pending_changes == 0 ||
			(locked = lc_try_lock_for_transaction(device->act_log)));
    atomic_set(&device->debug_noblock,2);
	if (locked) {
...
void __drbd_bitmap_set_in_sync(struct drbd_snapshot_device*device, sector_t sector, int size,
		       const char *file, const unsigned int line)
{
	/* Is called from worker and receiver context _only_ */
	unsigned long sbnr, ebnr, lbnr;
	...
	if (size <= 0 || !IS_ALIGNED(size, 512) || size > DRBD_MAX_DISCARD_SIZE) {
		drbd_err(device->device, "drbd_bitmap_set_in_sync: sector=%llus size=%d nonsense!\n",
				(unsigned long long)sector, size);
		return;
	}

	if (!get_ldev(device->device))
		return; /* no disk, no metadata, no bitmap to clear bits in */

	nr_sectors = drbd_get_capacity(device->ldev);
	esector = sector + (size >> 9) - 1;
	 ...
	 * we count rs_{total,left} in bits, not sectors.
	 */
	count = drbd_bitmap_clear_bits(device->bitmap, sbnr, ebnr);
	if (count) {
		//drbd_advance_rs_marks(device, drbd_bm_total_weight(device));
		//spin_lock_irqsave(&device->al_lock, flags);
	//drbd_try_clear_on_disk_bm(device, sector, count, true); //clear on disk bitmap
		//spin_unlock_irqrestore(&device->al_lock, flags);
...
void drbd_rs_complete_io(struct drbd_device *device, sector_t sector)
{
	...
	if (!bm_ext) {
		spin_unlock_irqrestore(&device->al_lock, flags);
		//dump_stack();//add by chengm,debug
		nc=rcu_dereference(first_peer_device(device)->connection->net_conf);
	    if(nc&&nc->wire_protocol==DRBD_PROT_S){ //add by chengm
            return ;
	    }
		if (DRBD_ratelimit(5*HZ, 5))
...
bool drbd_noblock_begin_io(struct drbd_device*device,unsigned long sector,unsigned long size){
    unsigned first= sector >>(AL_EXTENT_SHIFT -9);  // 4M		
    ...
    used=device->lru_noblock->used;
    if(used>=1500){
        set_current_state(TASK_INTERRUPTIBLE);
		schedule_timeout(SLEEP_TIME);// 100ms
    } 
    spin_lock_irq(&device->noblock_lock);
    for (enr = first; enr <= last; enr++){
        struct lc_element *extent;		
            extent = lc_get(device->lru_noblock, enr); // refcnt++, in_use or free
        if((extent)&&(extent->lc_number != enr)){	
           // lc_committed(device->lru_noblock);	// free to in_use
            extent->lc_number=extent->lc_new_number;
            list_move(&extent->list,&device->lru_noblock->in_use);
            device->lru_noblock->pending_changes--;
            
        }else if(!extent){	
            drbd_err(device,"get noblock lru failed! used=%d \n ",device->lru_noblock->used);	
            err=false;
        }			
    }		
    spin_unlock_irq(&device->noblock_lock);	
    return err;
}

bool drbd_noblock_compleate_io(struct drbd_device*device,unsigned long sector,unsigned long size,int cflags){
    unsigned first= sector >>(AL_EXTENT_SHIFT -9); //4M	
    unsigned last= size==0?first:(sector+(size>>9)-1) >> ( AL_EXTENT_SHIFT-9 );	
    unsigned enr;	
    struct lc_element *extent;
    ...
    spin_lock_irq(&device->noblock_lock);	
    for (enr = first; enr <= last; enr++) {	
        extent = lc_find(device->lru_noblock, enr);	 //in_use,lru
        if(extent) {
            if(cflags){ // clean
                int rnt=0;
                //unsigned long now;
               ...
               // now=jiffies;
                if(rnt){
                    extent->refcnt=1;
                    lc_put(device->lru_noblock,extent);
                    extent->lc_number=extent->lc_new_number=LC_FREE;
                    device->lru_noblock->starving++;  //repeat write mit
                    //if((jiffies-now)>10)
                     //   drbd_info(device,"remove rwmit extent[%ld,%d] cnt=%d jiffies:%ld \n",sector,size,rec,jiffies-now);
                }
            }else if(extent->refcnt){
                    if(0==lc_put(device->lru_noblock, extent)){  //refcnt--,  in_use-->lru
                        extent->lc_number=extent->lc_new_number=LC_FREE;
                    }
            }	
        }else{
