...
size_t lc_seq_printf_stats(struct seq_file *seq, struct lru_cache *lc)
{
	...
	return seq_printf(seq, "\t%s: used:%u/%u "
		"hits:%lu misses:%lu starving:%lu locked:%lu changed:%lu  pe_changed=%d flags=0x%lx\n",
		lc->name, lc->used, lc->nr_elements,
		lc->hits, lc->misses, lc->starving, lc->locked, lc->changed,lc->pending_changes,lc->flags);
}
