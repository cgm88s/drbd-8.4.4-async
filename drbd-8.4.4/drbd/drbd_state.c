	       ...
	       union drbd_state val, enum chg_state_flags f)
{
	...
	if (cl_wide_st_chg(device, os, ns)) {
		...
		if (drbd_send_state_req(first_peer_device(device), mask, val)) {
			...
			if (f & CS_VERBOSE)
				print_st_err(device, os, ns, rv);
			//atomic_set(&device->debug_noblock,3);	
			goto abort;
		}
        //atomic_set(&device->debug_noblock,4);
		wait_event(device->state_wait,
			(rv = _req_st_cond(device, mask, val)));
        //atomic_set(&device->debug_noblock,5);
		if (rv < SS_SUCCESS) {
			if (f & CS_VERBOSE)
				print_st_err(device, os, ns, rv);
		    //atomic_set(&device->debug_noblock,6);		
			goto abort;
		}
		spin_lock_irqsave(&device->resource->req_lock, flags);
		ns = apply_mask_val(drbd_read_state(device), mask, val);
				 ...
				 enum chg_state_flags flags)
{
    ...
    if((ns.conn!=os.conn)&&(ns.conn>=C_CONNECTED)){
         wake_up(&connection->sync_noblock_work.q_wait);	
    }
    if((ns.disk!=os.disk)&&(ns.disk>=D_INCONSISTENT)){
         wake_up(&connection->sync_noblock_work.q_wait);	
    }
    if((ns.pdsk!=os.pdsk)&&(ns.pdsk>=D_INCONSISTENT)){
         wake_up(&connection->sync_noblock_work.q_wait);	
    }
			   ...
			   union drbd_state ns, enum chg_state_flags flags)
{
    ...
    struct drbd_connection* conn=NULL;
		...
		if (get_ldev(device)) {
			if ((ns.role == R_PRIMARY || ns.peer == R_PRIMARY) &&
				...
				if (drbd_suspended(device)) {
				    ...
				    conn=first_connection(resource);
				    if(conn)
				        nc=rcu_dereference(conn->net_conf);
				    if(nc&&nc->wire_protocol!=DRBD_PROT_S){ // fix secony sync to primary    
...
static int w_after_conn_state_ch(struct drbd_work *w, int unused)
{
	...
	if (oc == C_STANDALONE && ns_max.conn == C_UNCONNECTED){
		...
		//if(connection->net_conf->wire_protocol==DRBD_PROT_S)
		//    drbd_thread_start(&connection->sync_noblock);
    }
