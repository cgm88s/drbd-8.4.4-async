...
struct kmem_cache *drbd_noblock_cache;  // trace noblock sync req...
...
//add debug for chengm	
/*int cmd=((struct p_header100 *)buf)->command;
	cmd=be16_to_cpu(cmd);
    if(cmd!=19&&cmd!=20){	
        printk(KERN_INFO" %s send cmd %d\n",__FUNCTION__,cmd);
//   dump_stack();
    }
   */ 
...
void drbd_init_set_defaults(struct drbd_device *device)
{
    ...
    atomic_set(&device->debug_noblock, 0);
    
	mutex_init(&device->own_state_mutex);
	device->state_mutex = &device->own_state_mutex;

	spin_lock_init(&device->al_lock);
	spin_lock_init(&device->noblock_lock);
	...
	init_waitqueue_head(&device->noblock_wait);
...
static void drbd_destroy_mempools(void)
{
    ...
    if(drbd_noblock_cache)
        kmem_cache_destroy(drbd_noblock_cache);
...
static int drbd_create_mempools(void)
{
    ...
    drbd_noblock_cache=kmem_cache_create("drbd_noblock",sizeof(struct lc_element),0,0,NULL);
    if (drbd_noblock_cache == NULL)
		goto Enomem;
...
static void drbd_destroy_device(struct kobject *kobj)
{
    ...
    lc_destroy(device->lru_noblock);//add for chengm@vanstor.com
...
struct drbd_connection *conn_create(const char *name, struct res_opts *res_opts)
{
    ...
    drbd_thread_init(resource, &connection->sync_noblock, drbd_sync_noblock, "sync_noblock");
	connection->sync_noblock.connection = connection;
	drbd_init_workqueue(&connection->sync_noblock_work);
...
enum drbd_ret_code drbd_create_device(struct drbd_config_context *adm_ctx, unsigned int minor)
{
    ...
    drbd_proc_create_device(disk->disk_name,device);
	return NO_ERROR;
...
void drbd_delete_device(struct drbd_device *device)
{
	...
	if(proc_sdrbd!=NULL)
	    remove_proc_entry(device->vdisk->disk_name,proc_sdrbd);
...
void __drbd_uuid_set(struct drbd_device *device, int idx, u64 val) __must_hold(local)
{
	...
	printk(KERN_INFO"set uuid[%d]:%016llx\n",idx,(unsigned long long)val);
	drbd_print_uuids(device, "set UUID");
}
...
void drbd_uuid_set(struct drbd_device *device, int idx, u64 val) __must_hold(local)
{
	...
	__drbd_uuid_set(device, idx, val);
	spin_unlock_irqrestore(&device->ldev->md.uuid_lock, flags);
	printk(KERN_INFO"move and set uuid[%d]:%016llx\n",idx,(unsigned long long)val);
	drbd_print_uuids(device, "move and set UUID");
}
