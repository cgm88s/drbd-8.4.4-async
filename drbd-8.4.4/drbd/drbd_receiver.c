...
int drbd_submit_peer_request(struct drbd_device *dev,
			     struct drbd_peer_request *peer_req,
			     const unsigned rw, int fault_type)
{
	...
	unsigned n_bios = 0;
	struct drbd_device *device=dev;
	struct drbd_bitmap *b=NULL;
	struct block_device*ldev=NULL;
	int snap_flags;
	struct drbd_snapshot_device*sdev=NULL;
	unsigned nr_pages = (ds + PAGE_SIZE -1) >> PAGE_SHIFT;
	int err = -ENOMEM;
	snap_flags=0;
    if(fault_type&DRBD_SNAP_DEV){
        fault_type&=(~DRBD_SNAP_DEV);
        snap_flags=1;
        sdev=(struct drbd_snapshot_device*)(dev->sdev);
        b=sdev->bitmap;
        ldev=sdev->ldev;
    }
...
submit:
	...
	if(1==snap_flags){
	    if(get_ldev(device)){
	        drbd_blk_run_queue(bdev_get_queue(ldev));
            put_ldev(device);
	    }
	}
	return 0;
			    ...
			    struct packet_info *pi) __releases(local)
{
	...
	if (!peer_req)
		goto fail;
    nc=rcu_dereference(peer_device->connection->net_conf);
    if((atomic_read(&device->rs_pending_cnt)<=0)&&(nc)&&(nc->wire_protocol==DRBD_PROT_S)){
...
static int receive_DataRequest(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	case P_RS_DATA_REQUEST:
		...
		if(nc&&nc->wire_protocol==DRBD_PROT_S){
		    drbd_noblock_begin_io(device,sector,size);
		}
		rcu_read_unlock();
		break;
					   ...
					   enum drbd_disk_state peer_disk) __must_hold(local)
{
    ...
    __acquire(local);
    if((device->ldev->md.flags&MDF_SNAPSHOT_SYNC_S)&&(device->sdev==NULL)){
...
static int do_snapshot_remote(struct drbd_connection *connection, struct packet_info *pi)
{
	struct drbd_peer_device *peer_device;
	struct drbd_device *device;
	struct p_sn_my *p = pi->data;
    unsigned int oflags;
    int ret=0;
    int retcode=0;
    int role=-1;
	...
	if(p->flags==P_SNAP_START){
	    device->ldev->md.flags|= MDF_SNAPSHOT_SYNC_T;
        drbd_md_sync(device);
        
        mutex_lock(&device->resource->adm_mutex);
        // drbd_state  mask{i=0,user_isp=user_isp_MASK},val{.i=0,user_isp=1}
        if (drbd_request_state(device, NS(user_isp, 1)) != SS_NOTHING_TO_DO){
              retcode=ERR_PAUSE_IS_SET;
        }
        mutex_unlock(&device->resource->adm_mutex);	
        drbd_info(device,"sync pause [%d] because while do snapshot device sync!\n",retcode);	
	}else if(p->flags==P_SNAP_END){
	    __acquire(local);
	    oflags=device->ldev->md.flags;
	    __release(local);
	    if(oflags&MDF_SNAPSHOT_SYNC_S){
	        role=1;
	        drbd_md_clear_flag(device,MDF_SNAPSHOT_SYNC_S);
	        //device->ldev->md.flags&= ~MDF_SNAPSHOT_SYNC_S;
	         wake_up(&connection->sync_noblock_work.q_wait);
	         drbd_md_sync(device);
	    }//if(oflags&MDF_SNAPSHOT_SYNC_T)
	    else if(oflags&MDF_SNAPSHOT_SYNC_T){
	        role=0;
	        ret=drbd_khelper_usercmd(device,"start");// do snapshot local
	        if(ret){
               drbd_err(device,"helper command: start do snapshot failed[%d]!\n",ret);
               printk(KERN_ERR"EVENT[0x00000000] helper command:start do snapshot failed!\n");
               //return ret; // fix bug for lvcreate -n -s failed ,not retry
               ret=0; // fix bug for drbdd:error receiving 
	        }
	        drbd_md_clear_flag(device,MDF_SNAPSHOT_SYNC_T);
	        //device->ldev->md.flags&= ~MDF_SNAPSHOT_SYNC_T;
	        drbd_md_sync(device);
        ...
        // sync resume
        mutex_lock(&device->resource->adm_mutex);
        if (drbd_request_state(device, NS(user_isp, 0)) != SS_NOTHING_TO_DO) {
            retcode=ERR_PAUSE_IS_CLEAR; // 135
        }
        mutex_unlock(&device->resource->adm_mutex);
        drbd_info(device,"sync resume [%d] as %s because snapshot device sync done!\n",retcode,(role==1)?"Primary":(role==0)?"Secondary":"Unknown");	
...
static struct data_cmd drbd_cmd_handler[] = {
	...
	[P_SNAP_START]       = {0,sizeof(struct p_sn_my),do_snapshot_remote },
	[P_SNAP_END]       = {0,sizeof(struct p_sn_my),do_snapshot_remote },
};
...
//printk(KERN_INFO"%s receiver drbd_cmd_handler[%d]\n",__FUNCTION__,pi.cmd);//chengm
		shs = cmd->pkt_size;
...
static int got_BlockAck(struct drbd_connection *connection, struct packet_info *pi)
{
	...
	if (p->block_id == ID_SYNCER) {
	    get_flags=get_ldev(device);
        if(get_flags){
            __acquire(local);
	        oflags=device->ldev->md.flags;
	        __release(local);
        }
        if((oflags&MDF_SNAPSHOT_SYNC_S)&&(device->sdev!=NULL)&&(device->sdev->flags==1)){
		  drbd_bitmap_set_in_sync(device->sdev, sector, blksize);
		  dec_rs_pending(device);
		}else{
		   rcu_read_lock();
		   nc=rcu_dereference(connection->net_conf);
		   if(get_flags&&nc&&nc->wire_protocol==DRBD_PROT_S){ // in_use
               if(drbd_noblock_compleate_io(device,sector,blksize,false)){
                    drbd_set_in_sync(device, sector, blksize);
               }
           }else{
                drbd_set_in_sync(device, sector, blksize);
           ...
           rcu_read_unlock();
		   dec_rs_pending(device);
		   if((oflags&MDF_SNAPSHOT_SYNC_S)&&(device->sdev!=NULL)){
...
static int got_NegRSDReply(struct drbd_connection *connection, struct packet_info *pi)
{
    ...
    nc=rcu_dereference(peer_device->connection->net_conf); // fix raid failed;read return error;
    if((atomic_read(&device->rs_pending_cnt)<=0)&&(nc)&&(nc->wire_protocol==DRBD_PROT_S)){
            inc_rs_pending(device);
    }
	dec_rs_pending(device);
...
int drbd_asender(struct drbd_thread *thi)
{
	...
	while (get_t_state(thi) == RUNNING) {
		...
		if (received == expect && cmd == NULL) {
			...
			//add by chengm
			/*if(pi.cmd!=19&&pi.cmd!=20){
			    printk(KERN_INFO" %s recv meta cmd %d\n",__FUNCTION__,pi.cmd);
			}*/
