...
BIO_ENDIO_TYPE drbd_request_endio BIO_ENDIO_ARGS(struct bio *bio, int error)
{
	...
	if (unlikely(error)) {
        ...
        if(bio_data_dir(bio) == WRITE){ 
            ...
            if(nc&&(nc->wire_protocol==DRBD_PROT_S)&&(connection->sync_noblock.t_state==RUNNING)){
	            drbd_noblock_compleate_io(device,req->i.sector, req->i.size,true);
	            drbd_set_out_of_sync(device, req->i.sector, req->i.size);
	            //printk(KERN_INFO"%s:wake up sync_noblock \n",__FUNCTION__);
                __acquire(local);
	            flags=device->ldev->md.flags;
	            __release(local);
	            if(!(flags&(MDF_SNAPSHOT_SYNC_S|MDF_SNAPSHOT_SYNC_T))){
                    wake_up(&connection->sync_noblock_work.q_wait);
                }
            }
            rcu_read_unlock();
        }
...
static int make_resync_request(struct drbd_device *device, int cancel)
{
    ...
    if(flags&MDF_SNAPSHOT_SYNC_T){
        mutex_lock(&device->resource->adm_mutex);
        // drbd_state  mask{i=0,user_isp=user_isp_MASK},val{.i=0,user_isp=1}
        if (_drbd_request_state(device, NS(user_isp, 1),CS_VERBOSE+CS_SERIALIZE) != SS_NOTHING_TO_DO){
            drbd_info(device,"start resync [%s] pause,because MDF_SNAPSHOT_SYNC_T!",device->state.conn==C_SYNC_TARGET?"target":"source");
        }	
        mutex_unlock(&device->resource->adm_mutex);	
        return 0;
    }
    if(flags&MDF_SNAPSHOT_SYNC_S){
        mutex_lock(&device->resource->adm_mutex);
        // drbd_state  mask{i=0,user_isp=user_isp_MASK},val{.i=0,user_isp=1}
        if (_drbd_request_state(device, NS(user_isp, 1),CS_VERBOSE+CS_SERIALIZE) != SS_NOTHING_TO_DO){
            drbd_info(device,"start resync [%s] pause,because MDF_SNAPSHOT_SYNC_S!",device->state.conn==C_SYNC_TARGET?"target":"source");
        }	
        mutex_unlock(&device->resource->adm_mutex);	
        snapshot_sync_setup(device);
        return 0 ;
    }
	if (!get_ldev(device)) {
...
int noblock_conn_check(enum drbd_conns s){
        if((s==C_CONNECTED)||(s==C_SYNC_SOURCE)){
            return true; // 1
        }
        return false;   // 0
}
int noblock_disk_check(int disk,int pdsk){
    if((disk<D_INCONSISTENT)||(pdsk<D_INCONSISTENT)){
        return false;
    }else{
        return true;
    }
...
//主动发送数据块到对端
unsigned long make_sync_request_send(struct drbd_device*device,int cancel){
    ...
    struct drbd_connection *connection=first_peer_device(device)->connection;
    struct net_conf *nc;
    int fault_type= DRBD_FAULT_RS_RD;
    ...
    rcu_read_unlock();
    if((0==noblock_conn_check(device->state.conn))||(0==noblock_disk_check(device->state.disk,device->state.pdsk))){
        return 0;
    }
    if(device->ldev->md.flags&(MDF_SNAPSHOT_SYNC_S|MDF_SNAPSHOT_SYNC_T)){
        return 0;
    }
    
    max_bio_size=queue_max_hw_sectors(device->rq_queue) << 9;  // 1M
    weight=drbd_bm_total_weight(device);	
    if(weight<1)
        return 0;
    for(i=0;i<weight;i++){
        if(!get_ldev(device)){ //
       ...
       // atomic_set(&device->debug_noblock,11+i);
        
    slow_sync:    
        if(device->ldev->md.flags&MDF_SNAPSHOT_SYNC_S){
            drbd_info(device,"noblock sync thread stop work because MDF_SNAPSHOT_SYNC_S!\n");
            put_ldev(device);
            return 0;
        }
        if((0==noblock_conn_check(device->state.conn))||(0==noblock_disk_check(device->state.disk,device->state.pdsk))){ // connection or disk close
            drbd_err(device, "%s:connection state changed during sync [%s]!\n",__FUNCTION__,drbd_conn_str(device->state.conn));
            put_ldev(device);
            return 0;
        }
        if(get_t_state(&connection->sync_noblock) != RUNNING){
            put_ldev(device);
            return 0;
        }
        mutex_lock(&first_peer_device(device)->connection->data.mutex);
        ...
        now=jiffies;
     next_sector:	  //扫瞄bitmap		
        size=BM_BLOCK_SIZE;  // 1<<12==4K
        
        bit=_drbd_bm_find_next(device,begin);
        if (bit == DRBD_END_OF_BITMAP) {
            put_ldev(device);
        ...
        sector = BM_BIT_TO_SECT(bit);
        begin=bit+1;
        if(drbd_bm_test_bit(device,bit)==0){
            goto next_sector;
        }
        ...
        /*if(DRBD_ratelimit(5*HZ,5)){
            drbd_info(device,"%s find the %ld[%d] request take %ld jiffies\n",__func__,i,size,jiffies-now);
        }*/    
        now=jiffies;
        ...
        spin_unlock_irq(&device->resource->req_lock);
        drbd_noblock_begin_io(device,sector,size);  // -->in_use
        if (drbd_submit_peer_request(device, peer_req, READ, fault_type) != 0){ // read io,
                goto submit_failed ;
        }
        //put_ldev(device); // READ end drbd_endio_read_sec_final->put_ldev()
        /*if(DRBD_ratelimit(5*HZ,5)){
            drbd_info(device,"\t %s send the %ld[%d] request take %ld jiffies\n",__func__,i,size,jiffies-now);
        }*/
    }
    //put_ldev(device);
...
submit_failed:	/* don't care for the reason here */
        drbd_noblock_compleate_io(device,sector,size,false);  // in_use-->lru
        drbd_err(device, "%s,submit failed, triggering re-connect\n",__FUNCTION__);	
        spin_lock_irq(&device->resource->req_lock);	
        list_del(&peer_req->w.list);
        spin_unlock_irq(&device->resource->req_lock);
out_free_e:        
...
int drbd_sync_noblock(struct drbd_thread*thi){	//sync_noblock线程
    ...
    idr_for_each_entry(&connection->peer_devices, peer_device, vnr) {
        struct drbd_device *device = peer_device->device;
        rcu_read_unlock();
        //wait_event_interruptible(connection->sync_noblock_work.q_wait,(device->state.conn>=C_CONNECTED));   
        while(get_t_state(&connection->sync_noblock) == RUNNING){
            //debug
           // atomic_set(&device->debug_noblock,1); //wait 1
            wait_event_interruptible(connection->sync_noblock_work.q_wait,(noblock_conn_check(device->state.conn))&&(1==noblock_disk_check(device->state.disk,device->state.pdsk)));
            if(get_t_state(&connection->sync_noblock) != RUNNING){
                break;
            }
            //debug device->debug_noblock=2;
           // atomic_set(&device->debug_noblock,2);   //wait 2
            wait_event_interruptible(connection->sync_noblock_work.q_wait,drbd_bm_total_weight(device)&&(!(device->ldev->md.flags&MDF_SNAPSHOT_SYNC_S)));
            //atomic_set(&device->debug_noblock,10); // normal ; running
            i=make_sync_request_send(device,0);
            //atomic_set(&device->debug_noblock,4);
            if(get_t_state(&connection->sync_noblock) != RUNNING){
                break;
            }
            //atomic_set(&device->debug_noblock,3); // wait 3
             wait_event_interruptible(connection->sync_noblock_work.q_wait,(!(device->ldev->md.flags&MDF_SNAPSHOT_SYNC_S)));   
        }
        //atomic_set(&device->debug_noblock,0); // exit 
        rcu_read_lock();
        if(get_t_state(&connection->sync_noblock) != RUNNING){
            break;
        }
...
int w_e_end_rsdata_req(struct drbd_work *w, int cancel)
{
	...
	if (get_ldev_if_state(device, D_FAILED)) {
	    if((device->ldev->md.flags&MDF_SNAPSHOT_SYNC_S)&&(device->sdev)&&(device->sdev->flags==1)){
	...
	if (device->state.conn == C_AHEAD) {
        ...
        if((nc)&&(nc->wire_protocol!=DRBD_PROT_S)){ // fix raid failed,diskless
		err = drbd_send_ack(peer_device, P_NEG_RS_DREPLY, peer_req);
        }else{
            drbd_noblock_compleate_io(device,peer_req->i.sector,peer_req->i.size,false);
        }
...
void drbd_start_resync(struct drbd_device *device, enum drbd_conns side)
{
	union drbd_state ns;
	int r;
    //unsigned long flags=device->ldev->md.flags;
