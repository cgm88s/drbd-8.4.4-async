...
static void drbd_syncer_progress(struct drbd_device *device, struct seq_file *seq)
{
	...
	if (!dt)
		dt++;
	if(device->rs_mark_left[i]-rs_left>0){ //add for chengm;fix S protocol -xxx	    
	...
	if (dt == 0)
		dt = 1;
	if((device->rs_total-rs_left)>0){   //add by chengm; fix S protocol -xxx
...
// our implement for show ip in kernel
char * inet_ntoa(struct in_addr ina,char*buf){
	...
	*/
	...
	idr_for_each_entry(&drbd_devices, device, i) {
		...
		if (device->state.conn == C_STANDALONE &&
			...
			bdi_rw_congested(&device->rq_queue->backing_dev_info); // .congested_fn=drbd_congested
			...
			noblockreq=atomic_read(&device->ap_pending_cnt)+
			            atomic_read(&device->rs_pending_cnt)+
			            atomic_read(&device->unacked_cnt)+
			            atomic_read(&device->ap_bio_cnt)+
			            atomic_read(&device->local_cnt);
			                
			seq_printf(seq, " oos:%llu.%lu\n",
				   Bit2KB((unsigned long long)
					   drbd_bm_total_weight(device)),(noblockreq));
			
			memcpy(&addr,&first_peer_device(device)->connection->my_addr,sizeof(addr));
			memcpy(&raddr,&first_peer_device(device)->connection->peer_addr,sizeof(raddr));
			if(device->ldev){
			    disk_conf=rcu_dereference(device->ldev->disk_conf);
			    if(disk_conf)
			        disk=disk_conf->backing_dev;
			}
		    seq_printf(seq,"    local:%s[%s] remote:%s ",inet_ntoa(addr.sin_addr,lip),disk,inet_ntoa(raddr.sin_addr,rip));
			if((wp=='S')&&(device->ldev)){
			    __acquire(local);
			    ...
			    if(flags&(MDF_SNAPSHOT_SYNC_S|MDF_SNAPSHOT_SYNC_T)){
                    seq_printf(seq," snap 1:snapshot sync running[%s]\n",flags&MDF_SNAPSHOT_SYNC_S?"Source":"Target");
			    }else if(device->state.role!=R_PRIMARY){
                    seq_printf(seq," snap 2:not support snapshot\n");
			    }else{
                    seq_printf(seq," snap 0:normal,noblock sync\n");
			    }
			}else{
                seq_printf(seq," snap 2:not support snapshot\n");
			}		   
