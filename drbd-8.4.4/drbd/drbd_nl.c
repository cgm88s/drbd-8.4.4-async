...
drbd_set_role(struct drbd_device *device, enum drbd_role new_role, int force)
{
	...
	if (new_role == R_SECONDARY) {
	    ...
	    if(nc&&(nc->wire_protocol==DRBD_PROT_S)&&(first_peer_device(device)->connection->sync_noblock.t_state==RUNNING)){
		    drbd_thread_stop(&first_peer_device(device)->connection->sync_noblock);
		}
		...
		if(nc&&nc->wire_protocol==DRBD_PROT_S)
		    drbd_thread_start(&first_peer_device(device)->connection->sync_noblock);
...
static int drbd_check_al_size(struct drbd_device *device, struct disk_conf *dc)
{
	...
	if(!(device->lru_noblock)){
    	device->lru_noblock=lc_create("noblock_sync",drbd_noblock_cache,1024,2000,sizeof(struct lc_element),0);
    	if (device->lru_noblock == NULL) {
    		drbd_err(device, "Cannot allocate noblock_sync lru!\n");
    		return -ENOMEM;
    	}
	}
...
int drbd_adm_attach(struct sk_buff *skb, struct genl_info *info)
{
	...
	set_disk_conf_defaults(new_disk_conf);
    //add by chengm@vanstor.com
    new_disk_conf->resync_rate=102400; // default is 250k
    new_disk_conf->c_max_rate=1024000; // default is 102400
    new_disk_conf->c_min_rate=250;      // default is 250k
...
int drbd_adm_connect(struct sk_buff *skb, struct genl_info *info)
{
	...
	//add by chengm
	if(new_net_conf->wire_protocol==DRBD_PROT_S){
        new_net_conf->max_buffers=8192; // default is 2048,max 1000000
    }
	retcode = check_net_options(connection, new_net_conf);
	if (retcode != NO_ERROR)
		goto fail;
...
static enum drbd_state_rv conn_try_disconnect(struct drbd_connection *connection, bool force)
{
	...
	if (rv >= SS_SUCCESS) {
		 ...
		 */
		drbd_thread_stop(&connection->receiver);
        //drbd_thread_stop(&connection->sync_noblock); //drbdadm down -->destroy sync_noblock
...
int drbd_adm_resume_sync(struct sk_buff *skb, struct genl_info *info)
{
    ...
    __acquire(local);
    oflags=adm_ctx.device->ldev->md.flags;
    __release(local);
    if(oflags&(MDF_SNAPSHOT_SYNC_S|MDF_SNAPSHOT_SYNC_T)){
        drbd_info(adm_ctx.device,"snapshot sync is running,not permit to resume sync!");
        goto out;
    }
	mutex_lock(&adm_ctx.resource->adm_mutex);
...
int drbd_adm_down(struct sk_buff *skb, struct genl_info *info)
{
	...
	for_each_connection(connection, resource) {
		struct drbd_peer_device *peer_device;
        struct drbd_snapshot_device*sd;
		idr_for_each_entry(&connection->peer_devices, peer_device, i) {
			...
			if(peer_device->device->sdev!=NULL){
			    sd=peer_device->device->sdev;
                if(sd->tsk!=NULL)
                    kthread_stop(sd->tsk);
			}
		}
	...
	for_each_connection(connection, resource){
		drbd_thread_stop(&connection->worker);
        drbd_thread_stop(&connection->sync_noblock);
     }
