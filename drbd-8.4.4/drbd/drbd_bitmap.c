...
void bm_free_pages(struct page **pages, unsigned long number)
{
	unsigned long i;
	if (!pages)
		return;
 ...
 */
struct page **bm_realloc_pages(struct drbd_bitmap *b, unsigned long want)
{
 ...
 */
#define BITS_PER_PAGE		(1UL << (PAGE_SHIFT + 3))  //1<<15
#define BITS_PER_PAGE_MASK	(BITS_PER_PAGE - 1)  //1<<15  -1
...
unsigned long bm_count_bits(struct drbd_bitmap *b)
{
...
static void bm_memset_unchanged(struct drbd_bitmap *b, size_t offset, int c, size_t len)
{
 ...
 */
int drbd_bm_resize(struct drbd_device *device, sector_t c_size, int set_new_bits)
{
	struct drbd_bitmap *b = device->bitmap;
	...
	int err = 0, growing;
	sector_t capacity=c_size-(c_size%32768); //fix bug; align to 16M
...
/* clear unchange page bits in the bitmap */
void drbd_bm_clear_unchanged(struct drbd_device *device)
{
	struct drbd_bitmap *b = device->bitmap;
	if (!expect(b))
		return;
	...
	spin_lock_irq(&b->bm_lock);
	bm_memset_unchanged(b, 0, 0, b->bm_words);
	spin_unlock_irq(&b->bm_lock);
	b->bm_set = drbd_bm_count_bits(device,0,b->bm_bits);
}

#define BM_SNAPDEV_FLAGS    8

struct bm_aio_ctx {
	struct drbd_device *device;
	struct drbd_bitmap*bitmap;      //add by chengm
/*	struct completion wait;         //add by chengm for sync io
	unsigned int wflags;            //add by chengm 
#define BM_SYNC_COPY_PAGES   1
#define BM_AIO_COPY_SNAPSHOT 2
*/
	atomic_t in_flight;
	unsigned int done;
	unsigned flags;
...
static BIO_ENDIO_TYPE bm_async_io_complete BIO_ENDIO_ARGS(struct bio *bio, int error)
{
	...
	struct drbd_bitmap *b = (ctx->flags & BM_SNAPDEV_FLAGS)?ctx->bitmap:device->bitmap;
    ...
    if(ctx->flags & BM_SNAPDEV_FLAGS){  //add by chengm@vanstor.com
...
static void bm_page_io_async_snapdevice(struct drbd_snapshot_device*sdevice,struct bm_aio_ctx *ctx, int page_nr, int rw) __must_hold(local)
{
	struct bio *bio = bio_alloc_drbd(GFP_NOIO);
	struct drbd_device *device = ctx->device;
	//action
	//struct drbd_snapshot_device*sdevice=cantainer_of(device,(struct drbd_snapshot_device),device);
	struct drbd_bitmap *b = sdevice->bitmap;
	struct page *page;
	unsigned int len;
...
/*
 * bm_rw_device: read/write the whole bitmap from/to its on disk location.
 */
int bm_rw_snapdevice(struct drbd_snapshot_device*sdevice, int rw, unsigned flags, unsigned lazy_writeout_upper_idx) __must_hold(local)
{
	...
	ctx = kzalloc(sizeof(struct bm_aio_ctx), GFP_NOIO);
	if (!ctx)
		return -ENOMEM;

	*ctx = (struct bm_aio_ctx) {
		.device = sdevice->device,
		.bitmap=sdevice->bitmap,
		.in_flight = ATOMIC_INIT(1),
		.done = 0,
		.flags = flags|BM_SNAPDEV_FLAGS,
		.error = 0,
		.kref = { ATOMIC_INIT(2) },
	};

	if (!get_ldev_if_state(sdevice->device, D_ATTACHING)) {  /* put is in bm_aio_ctx_destroy() */
		drbd_err(sdevice->device, "ASSERT FAILED: get_ldev_if_state() == 1 in bm_rw()\n");
		kfree(ctx);
		return -ENODEV;
	}
	...
	for (i = 0; i < num_pages; i++) {
		...
		if (rw & WRITE) {
			...
			if (!(flags & BM_WRITE_ALL_PAGES) &&
				...
				dynamic_drbd_dbg(sdevice->device, "skipped bm write for idx %u\n", i);
				continue;
			}
			...
			if (lazy_writeout_upper_idx &&
				...
				dynamic_drbd_dbg(sdevice->device, "skipped bm lazy write for idx %u\n", i);
				continue;
			}
		}
		atomic_inc(&ctx->in_flight);
		bm_page_io_async_snapdevice(sdevice,ctx, i, rw);
		++count;
		cond_resched();
	}
	 ...
	 */
	if (!atomic_dec_and_test(&ctx->in_flight)) { //
		drbd_blk_run_queue(bdev_get_queue(sdevice->ldev));
		wait_until_done_or_force_detached(sdevice->device, sdevice->device->ldev, &ctx->done);
	} else
		kref_put(&ctx->kref, &bm_aio_ctx_destroy);

	/* summary for global bitmap IO */
	if (flags == 0)
		drbd_info(sdevice->device, "snapdevice bitmap %s of %u pages took %lu jiffies\n",
			 rw == WRITE ? "WRITE" : "READ",
			 count, jiffies - now);

	if (ctx->error) {
		drbd_alert(sdevice->device, "snapdevice we had at least one MD IO ERROR during bitmap IO\n");
		drbd_chk_io_error(sdevice->device, 1, DRBD_META_IO_ERROR);
		err = -EIO; /* ctx->error ? */
	}
	...
	now = jiffies;
	if (rw == WRITE) { //
		drbd_md_flush_snapdevice(sdevice);
	} else /* rw == READ */ {
		b->bm_set = bm_count_bits(b);
		drbd_info(sdevice->device, "snapdevice recounting of set bits took additional %lu jiffies\n",
		     jiffies - now);
	}
	now = b->bm_set;

	if (flags == 0)
		drbd_info(sdevice->device, "snapdevice %s (%lu bits) marked out-of-sync by on disk bit-map.\n",
...
#ifdef COMPAT_KMAP_ATOMIC_PAGE_ONLY
#define __sbm_find_next(device, bm_fo, find_zero_bit, km) ___sbm_find_next(device, bm_fo, find_zero_bit)
static unsigned long ___sbm_find_next(struct drbd_snapshot_device *sdev, unsigned long bm_fo,
	const int find_zero_bit)
#else
static unsigned long __sbm_find_next(struct drbd_snapshot_device *sdev, unsigned long bm_fo,
	const int find_zero_bit, const enum km_type km)
#endif
{
	...
	if (bm_fo > b->bm_bits) {
		drbd_err(sdev->device, "snapshot dev bm_fo=%lu bm_bits=%lu\n", bm_fo, b->bm_bits);
		bm_fo = DRBD_END_OF_BITMAP;
	} else {
		while (bm_fo < b->bm_bits) {
			/* bit offset of the first bit in the page */
...
unsigned long _sdrbd_bm_find_next(struct drbd_snapshot_device *device, unsigned long bm_fo)
{
	/* WARN_ON(!(BM_DONT_SET & device->b->bm_flags)); */
	return __sbm_find_next(device, bm_fo, 0, KM_USER1);
}
...
static int __bitmap_change_bits_to(struct drbd_bitmap *bitmap, const unsigned long s,
	unsigned long e, int val)
{
	struct drbd_bitmap *b = bitmap;
	unsigned long *p_addr = NULL;
	unsigned long bitnr;
	unsigned int last_page_nr = -1U;
	int c = 0;
	...
	if (e >= b->bm_bits) {
		printk(KERN_ERR" BITMAP ASSERT FAILED: bit_s=%lu bit_e=%lu bm_bits=%lu\n",
				s, e, b->bm_bits);
		e = b->bm_bits ? b->bm_bits -1 : 0;
	}
	for (bitnr = s; bitnr <= e; bitnr++) {
...
static int bitmap_change_bits_to(struct drbd_bitmap *bitmap, const unsigned long s,
	const unsigned long e, int val)
{
	unsigned long flags;
	struct drbd_bitmap *b = bitmap;
	...
	c = __bitmap_change_bits_to(b, s, e, val);
...
/* returns number of bits changed 0 -> 1 */
int drbd_bitmap_set_bits(struct drbd_bitmap *bitmap, const unsigned long s, const unsigned long e)
{
	return bitmap_change_bits_to(bitmap, s, e, 1);
}

/* returns number of bits changed 1 -> 0 */
int drbd_bitmap_clear_bits(struct drbd_bitmap *bitmap, const unsigned long s, const unsigned long e)
{
	return -bitmap_change_bits_to(bitmap, s, e, 0);
}
...
int sdrbd_bm_test_bit(struct drbd_snapshot_device*sdev, const unsigned long bitnr)
{
	unsigned long flags;
	struct drbd_bitmap *b = sdev->bitmap;
	unsigned long *p_addr;
	...
	if (bitnr < b->bm_bits) {
		...
		drbd_err(sdev->device, "snapshot device bitnr=%lu > bm_bits=%lu\n", bitnr, b->bm_bits);
		i = 0;
	}
