...
void m__system(char **argv, int flags, const char *res_name, pid_t *kid, int *fd, int *ex)
{
	...
	if (pid == 0) { //子进程
		if (flags & RETURN_STDOUT_FD) {
			...
			dup2(pipe_fds[1], 1);   //输出1定向 写管道 
		}
		if (flags & RETURN_STDERR_FD) {
			close(pipe_fds[0]);    //关闭读
			dup2(pipe_fds[1], 2);  //错误2定向 写管道
		}
...
int adm_startup_primary(struct cfg_ctx *ctx){
        pid_t pid;
        struct d_resource*res=ctx->res;
        char *hostname=NULL;
        char *primary=NULL;
        if(res->become_primary_on){
            primary=res->become_primary_on->name;
        }
        if(res->me&&res->me->on_hosts){
            hostname=res->me->on_hosts->name;
        }
        if(hostname&&primary){
            if(0==strcmp(hostname,primary)){ 
                pid=fork();
                if(pid==0){
                    close(1); //output
                    close(2); //error
...
static int adm_up(struct cfg_ctx *ctx)
{
	...
	if (!current_res_name || strcmp(current_res_name, ctx->res->name)) {
		...
		schedule_deferred_cmd(adm_startup_primary,ctx,"primary",CFG_NET);
	}
